...
public:
    ...
    ColumnsDescription getTableStructure(const ASTPtr & ast_function, const Context & context) override;

private:
    StoragePtr executeImpl(const ASTPtr & ast_function, const Context & context, const std::string & table_name) const override;
    const char * getStorageTypeName() const override { return "Distributed"; }

    void prepare(const ASTPtr & ast_function, const Context & context) const;
