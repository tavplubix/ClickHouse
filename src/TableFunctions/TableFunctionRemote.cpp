...
void TableFunctionRemote::prepare(const ASTPtr & ast_function, const Context & context) const
{
    if (cluster)
        return;
...
ColumnsDescription TableFunctionRemote::getTableStructure(const ASTPtr & ast_function, const Context & context)
{
    prepare(ast_function, context);
