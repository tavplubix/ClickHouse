...
String Context::resolveDatabaseAndCheckAccess(const String & database_name) const
{
    ...
    if (res.empty())
        throw Exception("Default database is not selected", ErrorCodes::UNKNOWN_DATABASE);
    checkDatabaseAccessRightsImpl(res);
    return res;
}
...
{
    checkDatabaseAccessRights(database_name);
...
void Context::checkDatabaseAccessRightsImpl(const std::string & database_name) const
{
    ...
    /// Check it's not global _temporary_and_exteral_tables database. It can be access only by internal server queries
...
bool Context::isDictionaryExists(const String & database_name, const String & dictionary_name) const
{
    auto lock = getLock();
    String db = resolveDatabaseAndCheckAccess(database_name);
    auto db_ptr = shared->database_catalog->tryGetDatabase(database_name, *this);   // FIXME locks order
