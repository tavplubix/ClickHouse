...
bool ParserCreateViewQuery::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    ParserKeyword s_inner_table("INNER TABLE");
    ParserToken s_dot(TokenType::Dot);
    ParserToken s_lparen(TokenType::OpeningRoundBracket);
    ParserToken s_rparen(TokenType::ClosingRoundBracket);
    ParserStorage storage_p;
    ...
    if (ParserKeyword{"TO"}.ignore(pos, expected))
    {
        to_custom_table = is_materialized_view && s_inner_table.ignore(pos, expected);
    ...
    if (is_materialized_view && !to_custom_table)
    {
    ...
    query->to_table_id = to_table_id;
    query->to_custom_table = to_custom_table;
